export function HoverableCardFixed({ interactionMethod }: { interactionMethod: "mouse" | "keyboard" }) {
    ...
    return (
        <button
            className={`relative text-left w-full h-64 p-8 bg-cover rounded-lg text-white outline-offset-4 group flex flex-col justify-end overflow-clip cursor-default ${interactionMethod === "keyboard" ? `pointer-events-none` : ``}`}
        ...
        >
            <img
                ...
                alt=""
                className="absolute w-full top-0 left-0 -z-30 group-hover:scale-125 group-focus-within:scale-125 transition-all duration-300"
            />
